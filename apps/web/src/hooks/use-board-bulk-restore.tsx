...
export function useBoardBulkRestore({
  ...
  const bulkAnimation = useBulkAnimation({
    ...
    checkedDeletedItems: itemType === 'memo' 
      ? new Set(Array.from(checkedItems).filter(id => typeof id === 'number') as number[])
      : new Set(Array.from(checkedItems).filter(id => typeof id === 'number') as number[]),
  })

  // 共通の復元処理関数
  const executeRestoreWithAnimation = useCallback(async (
    ids: number[],
  ...
  }, [itemType, checkedItems, setCheckedItems, boardItems, deletedMemos, deletedTasks, restoreMemoMutation, restoreTaskMutation, bulkAnimation, setIsRestoring, setIsLidOpen])
  ...
  const handleBulkRestore = useCallback(async () => {
    ...
    // DOM順序でソート
    const { getMemoDisplayOrder, getTaskDisplayOrder } = await import('@/src/utils/domUtils');
    const domOrder = itemType === 'memo' ? getMemoDisplayOrder() : getTaskDisplayOrder();
  ...
  }, [checkedItems, boardItems, itemType, bulkRestore, executeRestoreWithAnimation, bulkAnimation, setIsRestoring, setIsLidOpen])
