...
- **PETABOOタスク関連のPlanは `.claude/plans/` に保存**
  - ファイル名形式: `PETABOO-XX_タスク名.md`
  - 例: `PETABOO-50_サイドバーアイコンのホームがまれに有効になり続けてしまう.md`
  - 内容: 問題の原因、調査結果、実装計画、影響範囲など
- **完了したPlanは `.claude/fixed-plans/` に移動**
...
### ステップ3: コミットメッセージのフォーマット

**チケット番号がある場合（PETABOO-XX形式）：**
...
- **ユーザーがチケット番号（PETABOO-XX）を指定した場合は、必ずコミットメッセージの先頭に付与すること**
