...
| 課題                         | 現状                      | 対応案                                      |
| ---------------------------- | ------------------------- | ------------------------------------------- |
| `["taggings", "all"]` 最適化 | 10回の invalidate         | setQueryData化                              |
| 残りの invalidate 削減       | 148箇所                   | 段階的に setQueryData 化                    |
| 競合時のフォーム処理         | 警告のみ、入力は破棄      | 下書き保存 or フォーム上書き                |
| refetch後のupdatedAt問題     | 古いupdatedAtで保存される | フォームにupdatedAt持たせる or 再取得時更新 |

### 優先度: 低（保存ボタン以外の操作）

以下は「保存ボタン」を使わない操作のため、統一の緊急性は低い。

| 操作     | 個人モード         | チームモード                  | 備考                       |
| -------- | ------------------ | ----------------------------- | -------------------------- |
| 作成     | ✅ updateItemCache | ⚠️ ボード系で invalidate 残り | 新規作成ボタン             |
| 削除     | ✅ updateItemCache | ⚠️ invalidate + refetch 残り  | 削除ボタン                 |
| 復元     | ✅ updateItemCache | ⚠️ invalidate + refetch 残り  | 復元ボタン                 |
| タグ付け | invalidate（許容） | invalidate（許容）            | タグ選択時、複数箇所に影響 |

**注**: 保存ボタン押下時（更新操作）は `use-simple-item-save.ts` → `updateItemCache` で統一済み。
...
| 指標                      | 修正前  | 修正後       | 変化   |
| ------------------------- | ------- | ------------ | ------ |
| invalidate/refetch        | 307箇所 | 148箇所      | -52%   |
| `["boards", "all-items"]` | 8回     | 2回          | -75%   |
| `["boards"]` 過剰         | 3回     | 0回          | -100%  |
| setQueryData              | 少数    | 53箇所       | 大幅増 |
| タスク一覧消えるバグ      | 発生    | 解消         | ✅     |
| 競合検出                  | なし    | 409+トースト | ✅     |
