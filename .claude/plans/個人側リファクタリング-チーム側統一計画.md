...
- [x] この計画書の作成
- [x] チーム側の既知バグリストアップ（PETABOO-44等で対応済み）
- [x] チーム側モバイル対応の完成
- [x] チーム側のボード詳細未保存確認の実装・検証
- [x] 個人側URL統一（PETABOO-33で対応: `/?SLUG&memo=N` 形式）
...
- 2025-12-04: Phase 1完了（チーム側完成、個人側URL統一）
  - PETABOO-33で個人側ボード詳細内のURL更新を実装
  - `/?SLUG&memo=N` / `/?SLUG&task=N` 形式でアイテム選択時のURL更新が動作
