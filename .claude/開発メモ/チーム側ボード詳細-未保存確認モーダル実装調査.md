# チーム側ボード詳細 - 未保存確認モーダル実装調査

**最終更新日**: 2025-11-21（モバイル版実装完了）

## 📋 調査概要

チーム側のPC版ボード詳細画面における、メモ/タスクエディターの未保存確認モーダルの実装状況を調査。

**結論**: PC版では正常に動作している ✅

---

## 🔍 実装構造

### 1. 未保存確認モーダルの配置

**場所**: `apps/web/components/features/memo/memo-editor.tsx:1629-1642`

```typescript
{/* 未保存変更確認モーダル */}
<ConfirmationModal
  isOpen={isCloseConfirmModalOpen}
  onClose={() => setIsCloseConfirmModalOpen(false)}
  onConfirm={handleConfirmClose}
  title="未保存の変更があります"
  message="保存せずに閉じると、変更内容が失われます。よろしいですか？"
  confirmText="閉じる"
  cancelText="キャンセル"
  variant="warning"
/>
```

### 2. トリガー処理

**場所**: `apps/web/components/features/memo/memo-editor.tsx:1140-1146`

```typescript
// 戻るボタンクリックハンドラー（未保存変更チェック）
const handleCloseClick = useCallback(() => {
  if (hasUnsavedChanges) {
    setIsCloseConfirmModalOpen(true); // モーダル表示
  } else {
    onClose(); // 直接閉じる
  }
}, [hasUnsavedChanges, onClose]);
```

### 3. 確認後の処理

**場所**: `apps/web/components/features/memo/memo-editor.tsx:1149-1158`

```typescript
// 確認モーダルで「閉じる」を選択
const handleConfirmClose = useCallback(() => {
  setIsCloseConfirmModalOpen(false);
  // 破棄が選択されたことを通知（保留中の選択を実行するため）
  dispatchDiscardEvent("memo");
  // 保留中の選択がない場合（戻るボタンの場合）はonCloseを呼ぶ
  // 少し遅延させてdispatchDiscardEventが処理されるのを待つ
  setTimeout(() => {
    onClose();
  }, 50);
}, [onClose]);
```

---

## 📂 関連ファイル構成

### エディターコンポーネント

- **MemoEditor**: `apps/web/components/features/memo/memo-editor.tsx`
- **TaskEditor**: `apps/web/components/features/task/task-editor.tsx`

### 画面コンポーネント

- **BoardDetailScreen3Panel**: `apps/web/components/screens/board-detail-screen-3panel.tsx`
- **TeamBoardDetailWrapper**: `apps/web/components/features/team/team-board-detail-wrapper.tsx`

### 共通ロジック

- **useUnsavedChangesGuard**: `apps/web/src/hooks/use-unsaved-changes-guard.ts`
- **useBoardOperations**: `apps/web/src/hooks/use-board-operations.ts`
- **TeamDetailContext**: `apps/web/src/contexts/team-detail-context.tsx`

### モーダルコンポーネント

- **ConfirmationModal**: `apps/web/components/ui/modals/confirmation-modal.tsx`

### 型定義

- **unsaved-changes**: `apps/web/src/types/unsaved-changes.ts`

---

## 🔄 動作フロー（PC版）

### A. 「閉じる」ボタンをクリックした場合

```
ユーザーがエディターの「閉じる」ボタンクリック
  ↓
handleCloseClick 実行
  ↓
hasUnsavedChanges をチェック
  ├─ true の場合
  │   ↓
  │  setIsCloseConfirmModalOpen(true)
  │   ↓
  │  ConfirmationModal 表示
  │   ├─「キャンセル」→ モーダルを閉じる
  │   └─「閉じる」
  │       ↓
  │      handleConfirmClose 実行
  │       ↓
  │      dispatchDiscardEvent("memo") 発火
  │       ↓
  │      setTimeout(() => onClose(), 50)
  │       ↓
  │      onClearSelection() 実行（親から渡された）
  │       ↓
  │      選択解除・URL更新
  │
  └─ false の場合
      ↓
     onClose() を直接実行
```

### B. 別のメモ/タスクを選択した場合

```
ユーザーが別のメモをクリック
  ↓
handleSelectMemo (use-board-operations.ts:266)
  ↓
handleSelectMemoWithGuard (use-unsaved-changes-guard.ts)
  ↓
getRefs() で TeamDetailContext から ref を取得
  ↓
memoEditorHasUnsavedChangesRef.current をチェック
  ├─ true の場合
  │   ↓
  │  pendingSelectionRef.current = 新しいメモ（保留）
  │   ↓
  │  memoEditorShowConfirmModalRef.current() 実行
  │   ↓
  │  setIsCloseConfirmModalOpen(true)
  │   ↓
  │  ConfirmationModal 表示
  │   └─「閉じる」
  │       ↓
  │      handleConfirmClose 実行
  │       ↓
  │      dispatchDiscardEvent("memo") 発火
  │       ↓
  │      useUnsavedChangesGuard の useEffect がキャッチ
  │       ↓
  │      pendingSelectionRef の値で onSelectItem(新しいメモ) 実行
  │       ↓
  │      新しいメモが選択される
  │
  └─ false の場合
      ↓
     onSelectItem(新しいメモ) を直接実行
```

---

## 🧩 未保存変更ガードシステム

### use-unsaved-changes-guard.ts の役割

**目的**: チーム/個人モードの両方で、未保存変更がある場合にアイテム選択をガードする

**主要機能**:

1. **Ref管理**: チーム/個人モードに応じて適切な ref を返す
2. **選択ガード**: 未保存変更がある場合、選択を保留してモーダル表示
3. **イベントリスナー**: `dispatchDiscardEvent` をキャッチして保留中の選択を実行

### TeamDetailContext による状態共有

**場所**: `apps/web/src/contexts/team-detail-context.tsx:23-27`

```typescript
// メモエディターの未保存変更状態とモーダル表示関数
memoEditorHasUnsavedChangesRef: React.MutableRefObject<boolean>;
memoEditorShowConfirmModalRef: React.MutableRefObject<(() => void) | null>;
// タスクエディターの未保存変更状態とモーダル表示関数
taskEditorHasUnsavedChangesRef: React.MutableRefObject<boolean>;
taskEditorShowConfirmModalRef: React.MutableRefObject<(() => void) | null>;
```

**エディター側での設定** (`memo-editor.tsx:1088-1096`):

```typescript
// チームモードの場合、Contextに最新の状態を反映
useEffect(() => {
  if (teamMode && teamDetailContext) {
    teamDetailContext.memoEditorHasUnsavedChangesRef.current =
      hasUnsavedChanges;
    teamDetailContext.memoEditorShowConfirmModalRef.current = () => {
      setIsCloseConfirmModalOpen(true);
    };
  }
}, [hasUnsavedChanges, teamMode, teamDetailContext]);
```

---

## 🎯 use-board-operations.ts での活用

**場所**: `apps/web/src/hooks/use-board-operations.ts:231-267`

```typescript
// 未保存変更ガード（メモ用）
const {
  personalHasUnsavedChangesRef: memoHasUnsavedChangesRef,
  personalShowConfirmModalRef: memoShowConfirmModalRef,
  handleSelectWithGuard: handleSelectMemoWithGuard,
} = useUnsavedChangesGuard<Memo | DeletedMemo>({
  itemType: "memo",
  teamMode: isTeamMode,
  teamDetailContext: teamDetailContext,
  onSelectItem: onSelectMemo || (() => {}),
  setScreenMode: (mode) => {
    if (mode === "view") {
      setRightPanelMode(null);
    }
  },
});

// 選択ハンドラー（未保存ガード付き）
const handleSelectMemo = handleSelectMemoWithGuard;

// 閉じる処理（ガード無し）
const handleCloseDetail = useCallback(() => {
  onClearSelection?.();
}, [onClearSelection]);
```

**重要**: `handleCloseDetail` は未保存ガードを通さないが、エディター自身の `handleCloseClick` で未保存チェックを行っているため問題なし。

---

## ✅ PC版の動作状況

### 正常に動作しているケース

1. ✅ **「閉じる」ボタンクリック時**
   - 未保存変更がある場合、モーダルが表示される
   - 「閉じる」選択でエディターが閉じる

2. ✅ **別のメモ/タスクを選択した時**
   - 未保存変更がある場合、モーダルが表示される
   - 「閉じる」選択で新しいアイテムが選択される

3. ✅ **新規作成中の切り替え**
   - 新規メモ作成中に別のメモを選択しようとした場合も適切にガード

### 未保存変更の判定ロジック

**場所**: `memo-editor.tsx` 内

```typescript
// 未保存変更の判定
const hasUnsavedChanges =
  hasChanges || // タイトル・内容の変更
  pendingBoardChanges; // ボード選択の変更
```

---

## 📝 まとめ

### 実装の特徴

1. **二段構え**:
   - エディター自身が「閉じる」ボタン用の確認機能を持つ
   - `useUnsavedChangesGuard` がアイテム選択時の確認を提供

2. **Context による状態共有**:
   - TeamDetailContext で未保存状態を共有
   - 複数のコンポーネントから参照可能

3. **イベント駆動**:
   - `dispatchDiscardEvent` でコンポーネント間通信
   - 保留中の選択を非同期で実行

4. **チーム/個人モード対応**:
   - `useUnsavedChangesGuard` が両モードに対応
   - 同じロジックを共有

### PC版の完成度

**現状**: ✅ 完全に動作している

- 未保存確認モーダルが適切に表示される
- ユーザーの選択（「閉じる」/「キャンセル」）が正しく処理される
- 状態管理が適切に行われている

---

---

## 🗑️ モバイル版未実装コードの削除作業

**実施日**: 2025-11-20

### 問題の背景

モバイル版では、サイドバーがフッターとして表示され、戻るボタンもそこに実装されている。PC版ではエディター内に閉じるボタンがあるためスムーズに未保存確認ができるが、モバイルではコンポーネントの階層が違いすぎて、以下の問題が発生していた：

- 変更の検知がうまくいかない
- Propsの受け渡しが複雑
- Contextの管理も困難
- **モーダルを出す前に閉じてしまう**

その試行錯誤の痕跡が残っていたため、一旦全て削除することにした。

### 削除したファイルと内容

#### 1. board-detail-screen.tsx (684-717行目)

```typescript
// ❌ 削除したコード
const [isMobileCloseConfirmOpen, setIsMobileCloseConfirmOpen] = useState(false);
const memoEditorHasUnsavedChangesRef = useRef(false);
const { mobileBackHandlerRef } = useNavigation();

const handleCloseDetailRef = useRef(handleCloseDetail);
useEffect(() => {
  handleCloseDetailRef.current = handleCloseDetail;
}, [handleCloseDetail]);

// モバイル戻るボタンハンドラー（BoardDetailScreen側で登録）
useEffect(() => {
  if (!isMobile || !selectedMemo) {
    mobileBackHandlerRef.current = null;
    return;
  }

  const handleMobileBack = () => {
    if (memoEditorHasUnsavedChangesRef.current) {
      setIsMobileCloseConfirmOpen(true);
    } else {
      handleCloseDetailRef.current();
    }
  };

  mobileBackHandlerRef.current = handleMobileBack;

  return () => {
    mobileBackHandlerRef.current = null;
  };
}, [isMobile, selectedMemo, mobileBackHandlerRef]);
```

```typescript
// ❌ 削除したモーダル（レンダリングされていなかった）
<ConfirmationModal
  isOpen={isMobileCloseConfirmOpen}
  onClose={() => setIsMobileCloseConfirmOpen(false)}
  onConfirm={() => {
    setIsMobileCloseConfirmOpen(false);
    handleCloseDetailRef.current();
  }}
  title="未保存の変更があります"
  message="編集中の内容が保存されていません。破棄してもよろしいですか？"
  confirmText="破棄して閉じる"
  cancelText="編集に戻る"
  variant="warning"
  icon="warning"
/>
```

- ❌ `ConfirmationModal` のimport削除
- ❌ `useNavigation` のimport削除
- ❌ MemoScreenへの未使用refのprops削除

**問題点**: モーダル自体がレンダリングされておらず、`memoEditorHasUnsavedChangesRef`がMemoEditorに渡されていないため、完全に動作していなかった。

#### 2. team-detail.tsx (514-522行目)

```typescript
// ❌ 削除したコード
const handleBackToTaskList = (_event: CustomEvent) => {
  // 未保存変更がある場合は確認モーダルを表示
  if (
    taskEditorHasUnsavedChangesRef.current &&
    taskEditorShowConfirmModalRef.current
  ) {
    taskEditorShowConfirmModalRef.current();
    return;
  }

  // タスクの選択を解除してタスク一覧に戻る
  // ...
};
```

**問題点**: TaskEditorが`mobileBackHandlerRef`を使っていないため動作せず、`handleBackToMemoList`には未保存確認が無く不整合だった。

#### 3. memo-editor.tsx (1108-1131行目)

```typescript
// ❌ 削除したコード
// モバイルフッター戻るボタンハンドラー（ref経由・超シンプル版）
// 注意: memoEditorHasUnsavedChangesRefが渡されている場合は親が管理するため登録しない
useEffect(() => {
  // 親が管理する場合はスキップ
  if (memoEditorHasUnsavedChangesRef) {
    return;
  }

  const handleMobileBack = () => {
    const currentHasUnsavedChanges = hasUnsavedChangesRef.current;

    if (currentHasUnsavedChanges) {
      setIsCloseConfirmModalOpen(true);
    } else {
      onCloseRef.current();
    }
  };

  mobileBackHandlerRef.current = handleMobileBack;

  return () => {
    mobileBackHandlerRef.current = null;
  };
}, [memoEditorHasUnsavedChangesRef]);
```

- ❌ `mobileBackHandlerRef` の取得削除
- ❌ `showingBoardDetail` の取得削除（未使用）
- ❌ `useNavigation` のimport削除

**問題点**: 個人側専用の実装だったが動作しておらず、チーム側では別の仕組み（TeamDetailContext）を使用していた。

#### 4. navigation-context.tsx

```typescript
// ❌ 削除した型定義
interface NavigationContextType {
  // ...
  mobileBackHandlerRef: React.MutableRefObject<(() => void) | null>; // ❌ 削除
}
```

```typescript
// ❌ 削除したref作成
const mobileBackHandlerRef = useRef<(() => void) | null>(null);
```

```typescript
// ❌ 削除したProvider値
<NavigationContext.Provider
  value={{
    // ...
    mobileBackHandlerRef, // ❌ 削除
  }}
>
```

- ❌ 不要な `useRef` のimport削除

#### 5. sidebar.tsx (240-252行目)

```typescript
// ❌ 削除したコード
onBack={() => {
  // チームモードの場合はメモ一覧に戻るイベントを発火
  if (isTeamMode) {
    window.dispatchEvent(new CustomEvent("team-back-to-memo-list"));
  } else {
    // 個人モードの場合はrefから直接ハンドラーを呼ぶ
    if (mobileBackHandlerRef.current) {
      mobileBackHandlerRef.current();
    }
  }
}}
```

```typescript
// ✅ 修正後（TODOコメント追加）
onBack={() => {
  // チームモードの場合はメモ一覧に戻るイベントを発火
  if (isTeamMode) {
    window.dispatchEvent(new CustomEvent("team-back-to-memo-list"));
  }
  // TODO: 個人モードの戻るボタン処理（未実装）
}}
```

- ❌ `mobileBackHandlerRef` の取得削除

### 削除結果

- TypeScriptエラー: **0件** ✅
- Lintエラー: **0件** ✅
- モバイル関連の未使用コード: **完全削除** ✅

### 残っているもの

1. **PC版の未保存確認モーダル**: 正常動作中 ✅
2. **チーム側のイベント駆動処理**: `team-back-to-memo-list` イベント（正常動作）✅
3. **個人側モバイルの戻るボタン**: TODOコメントのみ（未実装）

### 今後の方針

モバイル版の未保存確認機能は、PC版と同じ仕組みでは実装が困難であることが判明。フッターの階層構造を考慮した新しいアプローチが必要。

---

## ✅ モバイル版実装完了（2025-11-21）

### 実装アプローチ

**解決策**: Sidebar（フッター）から TeamDetailContext 経由で未保存状態を直接チェック

以前の問題点であった「階層の違いによりモーダルを出す前に閉じてしまう」問題を、**Context経由での状態参照**により解決。

### 実装内容

#### 1. Sidebar に未保存チェックを追加

**場所**: `apps/web/components/layout/sidebar.tsx`

```typescript
// チームモードの未保存確認用（Provider外でも安全に使用可能）
const teamDetailContext = useTeamDetailSafe();

// メモエディター用 onBack
onBack={() => {
  if (isTeamMode) {
    // Context から未保存状態を確認
    if (teamDetailContext) {
      const hasUnsaved = teamDetailContext.memoEditorHasUnsavedChangesRef.current;
      if (hasUnsaved) {
        // 未保存変更あり → モーダル表示して終了
        teamDetailContext.memoEditorShowConfirmModalRef.current?.();
        return;
      }
    }
    // 未保存変更なし → 通常の閉じる処理
    const backEventName = showingBoardDetail
      ? "board-memo-back"
      : "team-back-to-memo-list";
    window.dispatchEvent(new CustomEvent(backEventName));
  }
}}
```

#### 2. MemoEditor の handleConfirmClose を修正

**場所**: `apps/web/components/features/memo/memo-editor.tsx`

```typescript
const handleConfirmClose = useCallback(() => {
  setIsCloseConfirmModalOpen(false);
  dispatchDiscardEvent("memo");

  // モバイル版チームモードの場合はイベント発火で閉じる
  const isMobile = typeof window !== "undefined" && window.innerWidth < 768;
  if (isMobile && teamMode) {
    // ボード詳細画面かどうかをURLから判断
    const isFromBoardDetail =
      typeof window !== "undefined" &&
      window.location.search.includes("tab=board");
    const backEventName = isFromBoardDetail
      ? "board-memo-back"
      : "team-back-to-memo-list";
    window.dispatchEvent(new CustomEvent(backEventName));
  }
}, [teamMode]);
```

#### 3. MemoEditor の useEffect にクリーンアップ追加

```typescript
useEffect(() => {
  if (teamMode && teamDetailContext) {
    teamDetailContext.memoEditorHasUnsavedChangesRef.current =
      hasUnsavedChanges;
    teamDetailContext.memoEditorShowConfirmModalRef.current = () => {
      setIsCloseConfirmModalOpen(true);
    };
  }
  // クリーンアップ: アンマウント時にrefをリセット
  return () => {
    if (teamMode && teamDetailContext) {
      teamDetailContext.memoEditorHasUnsavedChangesRef.current = false;
      teamDetailContext.memoEditorShowConfirmModalRef.current = null;
    }
  };
}, [hasUnsavedChanges, teamMode, teamDetailContext]);
```

#### 4. layout.tsx のフッターを Sidebar に統一

**変更前**: ボード詳細画面では layout.tsx で直接 ItemEditorFooter を3種類配置

**変更後**: すべてのケースで Sidebar を使用

```typescript
// モバイル用ボトムナビ：統一してSidebarを使用
<div className="md:hidden fixed bottom-0 left-0 right-0 h-14 border-t border-gray-200 bg-white z-50">
  <Sidebar
    // ...既存props
    activeBoardSection={activeBoardSection}
    onBoardSectionChange={(section) => {
      setActiveBoardSection(section);
      window.dispatchEvent(
        new CustomEvent("board-section-change", { detail: { section } }),
      );
    }}
  />
</div>
```

#### 5. Sidebar に新しい props を追加

```typescript
interface SidebarProps {
  // ...既存props
  taskImageCount?: number;
  taskCommentCount?: number;
  // ボード詳細用
  activeBoardSection?: "memos" | "tasks" | "comments";
  onBoardSectionChange?: (section: "memos" | "tasks" | "comments") => void;
}
```

### 処理フロー（モバイル版）

```
戻るボタンクリック（Sidebar の onBack）
  ↓
teamDetailContext.memoEditorHasUnsavedChangesRef.current をチェック
  ├─ true の場合
  │   ↓
  │  teamDetailContext.memoEditorShowConfirmModalRef.current() 実行
  │   ↓
  │  MemoEditor 内でモーダル表示
  │   └─「閉じる」ボタンクリック
  │       ↓
  │      handleConfirmClose 実行
  │       ↓
  │      board-memo-back または team-back-to-memo-list イベント発火
  │       ↓
  │      エディターが閉じる
  │
  └─ false の場合
      ↓
     board-memo-back または team-back-to-memo-list イベント発火
      ↓
     エディターが閉じる
```

### 変更ファイル一覧

| ファイル          | 変更内容                                              |
| ----------------- | ----------------------------------------------------- |
| `sidebar.tsx`     | 未保存チェック追加、ボード用フッター追加、新props追加 |
| `memo-editor.tsx` | handleConfirmClose修正、useEffectクリーンアップ追加   |
| `task-editor.tsx` | 同上（タスク版）                                      |
| `layout.tsx`      | ボード詳細フッターを削除しSidebarに統一               |

### 実装結果

- ✅ **メモ一覧からのメモ編集** - 未保存確認モーダル表示
- ✅ **タスク一覧からのタスク編集** - 未保存確認モーダル表示
- ✅ **ボード詳細からのメモ編集** - 未保存確認モーダル表示
- ✅ **ボード詳細からのタスク編集** - 未保存確認モーダル表示
- ✅ **モーダルで「閉じる」選択後** - 正常にエディターが閉じる
- ✅ **モーダルで「キャンセル」選択後** - エディター継続
- ✅ **PC版の動作** - 影響なし

### 設計のポイント

1. **Context による状態共有**: TeamDetailContext の ref を使用し、階層を超えた状態参照を実現
2. **フッターの統一**: layout.tsx の直接実装を削除し、Sidebar に一元化
3. **イベント駆動**: `board-memo-back` / `team-back-to-memo-list` イベントで閉じる処理を統一
4. **クリーンアップ**: エディターアンマウント時に ref をリセットし、状態の不整合を防止

---

## 参考リンク

- [個人側リファクタリング-チーム側統一計画.md](./個人側リファクタリング-チーム側統一計画.md)
- ConfirmationModal: `apps/web/components/ui/modals/confirmation-modal.tsx`
