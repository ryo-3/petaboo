...
function TeamSettingsScreen({ teamId }: TeamSettingsScreenProps) {
  ...
  const [teamDescription, setTeamDescription] = useState(mockTeamData.description);
  const [inviteEmail, setInviteEmail] = useState("");
  const [inviteRole, setInviteRole] = useState<"member" | "admin">("member");

  const tabItems = [
    { id: "members" as const, label: "メンバー", icon: <Users className="w-4 h-4" /> },
    { id: "invitations" as const, label: "招待管理", icon: <Mail className="w-4 h-4" /> },
    { id: "permissions" as const, label: "権限設定", icon: <Shield className="w-4 h-4" /> },
    { id: "general" as const, label: "基本設定", icon: <Settings className="w-4 h-4" /> }
  ];
      ...
      <div className="flex-1 overflow-auto p-5">
        ...
        {activeTab === "members" && (
          <div className="space-y-6">
            <div>
              <h2 className="text-lg font-semibold text-gray-900 mb-4">メンバー一覧</h2>
                  ...
                  {mockTeamData.members.map((member) => (
                    <div key={member.id} className="px-4 py-3">
                      <div className="grid grid-cols-4 gap-4 items-center">
                        ...
                        <div className="text-sm text-gray-600">{member.joinedAt}</div>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {activeTab === "invitations" && (
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">新しいメンバーを招待</h3>
                    ...
                    {mockTeamData.invitations.map((invitation) => (
                      <div key={invitation.id} className="px-4 py-3">
                        <div className="grid grid-cols-5 gap-4 items-center">
                          ...
                          <div className="text-sm text-gray-600">{invitation.invitedBy}</div>
                          <div className="text-sm text-gray-600">{invitation.createdAt}</div>
                          <div>
        ...
        {activeTab === "general" && (
          <div className="space-y-6">
            <div>
              <h2 className="text-lg font-semibold text-gray-900 mb-4">チーム基本設定</h2>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    チーム名
