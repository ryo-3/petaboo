...
app.put("/:userId", async (c) => {
  try {
    ...
    if (
      memoColumnCount !== undefined &&
      (typeof memoColumnCount !== "number" ||
        memoColumnCount < 1 ||
        memoColumnCount > 4)
    ) {
    ...
    if (
      taskColumnCount !== undefined &&
      (typeof taskColumnCount !== "number" ||
        taskColumnCount < 1 ||
        taskColumnCount > 4)
    ) {
    ...
    if (
      memoHideControls !== undefined &&
      typeof memoHideControls !== "boolean"
    ) {
      return c.json({ error: "Invalid memo hide controls value" }, 400);
    }

    if (
      taskHideControls !== undefined &&
      typeof taskHideControls !== "boolean"
    ) {
    ...
    if (taskHideControls !== undefined)
      updateData.taskHideControls = taskHideControls;
