...
### React Query設定統一 🔄 **進行中**（2025-10-10）

- [x] 計画策定
- [x] `staleTime: 0`削除（use-boards.ts 2箇所修正）
- [x] チーム用hooks `refetchInterval`追加（1分ごと自動更新）
  - use-team-memos.ts（useTeamMemos, useDeletedTeamMemos）
  - use-team-tasks.ts（useTeamTasks, useDeletedTeamTasks）
