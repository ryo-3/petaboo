...
- 既存データ連番付与は新規作成時に自動生成されるため不要

### Phase 6: 🔧 準備完了（本番実行待ち）

- マイグレーションSQL作成完了: `apps/api/drizzle/0028_drop_original_id_columns_phase6.sql`
- スキーマ定義修正完了（8ファイル）
- API実装クリーンアップ完了
- 本番実行手順書作成完了: `.claude/plans/phase6-migration-manual.md`

**次のステップ**: 本番環境へのデプロイ後、Phase 6マイグレーション実行
