...
export function useTasksBulkDelete({
  ...
  useEffect(() => {
    if (tasks && !bulkAnimation.isPartialProcessing) {
      ...
      const newCheckedTasks = new Set(
        Array.from(checkedTasks).filter((id) => allTaskIds.has(id)),
      );
  ...
  const handleBulkDelete = async () => {
    ...
    const currentTabTaskIds = rawTargetIds.filter((id) =>
    ...
    if (isLimitedDelete) {
      ...
      await bulkDelete.confirmBulkDelete(
        ...
        <TaskDeleteMessage
          taskIds={rawTargetIds}
          currentTabTaskIds={targetIds}
        />,
  ...
  const DeleteModal = () => {
    const customTitle =
      activeTab === "deleted" ? "タスクの完全削除" : "タスクを削除";
  ...
  const getCurrentTabDeleteCount = () => {
    ...
    const allTasks = tasks || [];
    const currentTabTasks = allTasks.filter(
      (task) => task.status === activeTab,
    );
    const currentTabTaskIds = currentTabTasks.map((task) => task.id);
    const selectedCurrentTabTasks = Array.from(checkedTasks).filter((id) =>
      currentTabTaskIds.includes(id),
    );
