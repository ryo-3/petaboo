...
export default function TagRemoveModal({
  ...
  const [selectedTagIdsForRemove, setSelectedTagIdsForRemove] = useState<number[]>([]);
  ...
  const handleRemoveTags = useCallback(async () => {
    if (selectedItems.length === 0 || selectedTagIdsForRemove.length === 0) return;
      ...
      if (newProcessedCount >= selectedItems.length) {
        ...
        selectedItems.forEach(itemId => {
          ...
          if (item) {
            ...
            console.log('🔄 キャッシュ無効化 (削除):', { itemType, originalId });
          }
        });
  ...
  }, [selectedItems, selectedTagIdsForRemove, allItems, itemType, deleteTaggingsByTag, onSuccess, processedCount, queryClient]);
      ...
      footer={
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            disabled={selectedTagIdsForRemove.length === 0 || isRemovingTags || isCompleted || availableTagsForRemove.length === 0}
          >
            {isCompleted ? '削除完了' : isRemovingTags 
              ? '削除中...'
