...
重要な設計や実装の詳細は `.claude/開発メモ/` ディレクトリに保存されています。

### 主要ドキュメント

- **[モバイルフッター切り替えロジック.md](.claude/開発メモ/モバイルフッター切り替えロジック.md)** - モバイル版のフッター切り替え、イベントシステム、新規作成・戻る処理の完全ガイド
- **[メモ構造について.md](.claude/開発メモ/メモ構造について.md)** - メモシステムの設計とデータ構造
- **[キャッシュ設計について.md](.claude/開発メモ/キャッシュ設計について.md)** - React Queryのキャッシュ戦略

その他のドキュメントは `.claude/開発メモ/` を参照してください。
