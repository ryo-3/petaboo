...
function ItemGrid({ viewMode, effectiveColumnCount, children, isBoard = false }: ItemGridProps) {
  const getGridClassName = () => {
    ...
    if (viewMode === 'card') {
      ...
      return `${
        ...
        effectiveColumnCount === 2 ? `grid grid-cols-2 ${gapXClass}` :
        effectiveColumnCount === 3 ? `grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 ${gapXClass}` :
  ...
  return (
    <div className={`flex-1 overflow-auto ${isBoard ? '' : 'pr-2'} pb-10 mb-2`}>
